...
export class MeetingDetailsPage implements OnInit {
  ...
  getMeeting(meetingStateData) {
    // this.meeting = null;
    this.meetingsSubs$ = this.meetingservice
      .getMeetingById(meetingStateData.id)
      .subscribe((act) => {
  ...
  // editMeeting
  editMeeting() {
    this.router.navigate(["meeting/meeting-details-edit"], {
      state: { data: { meeting: this.meeting } },
    });
  }
  // share minutes
  async sendMinutes() {
    let response = await this.meetingservice.shareMeetingMinutes(
      this.meeting,
      this.alllinkages
    );
    let buttons = [
