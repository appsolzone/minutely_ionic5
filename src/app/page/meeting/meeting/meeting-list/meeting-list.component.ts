...
export class MeetingListComponent implements OnInit {
  ...
  constructor(
    ...
    this.sessionSubs$ = this.session.watch().subscribe(async (value) => {
      ...
      if (
        this.sessionInfo?.uid != value?.uid ||
        this.sessionInfo?.subscriberId != value?.subscriberId
      ) {
      ...
      if (
        this.sessionInfo?.subscriberId &&
        this.sessionInfo?.uid &&
  ...
  async getMeetings() {
    ...
    queryObj = [
      {
        field: "subscriberId",
        operator: "==",
        value: this.sessionInfo.subscriberId,
      },
    ];

    if (!this.searchText?.trim() && this.searchModeUser == "user") {
      queryObj.push({
        field: "attendeeUidList",
        operator: "array-contains-any",
        value: [this.sessionInfo.uid],
      });
    } else if (this.searchText?.trim() && this.searchModeUser == "user") {
      queryObj.push({
        field: "searchMap." + this.sessionInfo.uid,
        operator: "==",
        value: true,
      });
    }
    ...
    if (this.searchText?.trim().length >= 3) {
      searchTextObj = {
        seachField: "searchMap",
        text: this.searchText.trim(),
        searchOption: this.searchMode,
      };
    }
    console.log(
      "queryObj, searchTextObj, this.limit",
      queryObj,
      searchTextObj,
      this.limit
    );
    if (this.searchText?.trim().length >= 3 || this.dateRange.startDate) {
      // or dates selected
      this.viewMeetingResult = null;
      this.meetingsSubs$ = this.meeting
        .getMeetings(queryObj, searchTextObj, this.limit)
        .subscribe((act) => {
          let allMeetings = [];
          act.forEach((a: any) => {
            ...
            const meetingEnd = data.meetingEnd?.seconds
              ? new Date(data.meetingEnd?.seconds * 1000)
              : null;
            ...
            if (idx == -1) {
              allMeetings.push({
                id,
                data: { ...data, meetingStart, meetingEnd },
              });
            }
          });
          // this.viewMeetingResult =[];
          this.viewMeetingResult = allMeetings.sort(
            (a: any, b: any) => a.data.meetingStart - b.data.meetingStart
          );
  ...
  repopulateMeetingList() {
    ...
    if (this.viewMeetingResult) {
      this.viewMeetingResult.forEach((a: any) => {
        ...
        // return {id, data: { ...data, startTime, endTime }};
        if (idx == -1) {
          const meetingStart = data.meetingStart?.seconds
            ? new Date(data.meetingStart?.seconds * 1000)
            : data.meetingStart
            ? data.meetingStart
            : null;
          const meetingEnd = data.meetingEnd?.seconds
  ...
  openMeetingDetails(meeting) {
    if (this.viewMode != "linkage") {
      this.router.navigate(["meeting/meeting-details"], {
        state: { data: { meeting: meeting } },
      });
    }
  }
  ...
  public meetingTour() {
    console.log("calling Meeting tour");
    // console.log(document.getElementsByClassName("sm_create_meeting"));
    // console.log(document.getElementsByClassName("sm_create_meeting").length);
    // console.log(document.getElementById("sm_create_meeting"));
    // console.log(document.getElementsByClassName("md_create_meeting"));
    // console.log(document.getElementsByClassName("md_create_meeting").length);
    // console.log(document.getElementById("md_create_meeting"));

    // setTimeout(() => {
    this.meetingGuide.navigateToMeeting(this.gotoAddMeeting.bind(this));
    // }, 300);
  }
}
