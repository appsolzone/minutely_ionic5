...
@Component({
  selector: 'app-risk-details-edit',
  templateUrl: './risk-details-edit.page.html',
  styleUrls: ['./risk-details-edit.page.scss'],
})
@Autounsubscribe()
export class RiskDetailsEditPage implements OnInit {
  // observables
  sessionSubs$;
  risksSubs$;
  ...
  getrisk(riskStateData){
    ...
    const riskInitiationDate = moment(data.riskInitiationDate).format('YYYY-MM-DD');
  ...
  async saveRisk(){
    ...
    if(!validation.status){
      ...
      title = 'Confirmation';
      body = "You are about to save changes for the risk. Are you sure that you want to continue to update the risk?";
      response = false;
