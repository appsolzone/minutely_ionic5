...
export class RegistrationService {
  ...
  registerSubscriber(
    ...
    const aclKpiRef = this.db.afs.firestore
      ...
      .doc(sId);
    const minutelykpiRef = this.db.afs.firestore.collection(this.db.allCollections.minutelykpi).doc(sId);
    ...
    return this.db.afs.firestore.runTransaction((transaction) => {
      return transaction.get(subscriberRef).then((subsDoc) => {
        if (subsDoc.exists) {
          ...
          // insert initial minutelykpi object
          this.db.setTransactDocument(transaction, minutelykpiRef, this.kpi.kpiObj);
        }
